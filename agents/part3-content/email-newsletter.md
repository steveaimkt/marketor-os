...
6. **발송** · 승인되면 다음 두 경로 중 하나로 발송. 미승인이면 발송하지 않고 종료.
   ...
   - **(완전 자동) SMTP 발송** · 메일 발송 도구(직접 연결) 로 즉시 실발송. cron 연동 시 무인 발송. → 아래 "완전 자동 발송" 참조.
...
0 9 * * 1  cd /path/to/marketing-os && \
  claude -p "email-newsletter 에이전트로 이번 주 트렌드를 직접 검색해 뉴스레터를 작성하고 대표 메일 로 발송" \
...
**발송 스크립트** · 메일 발송 도구(직접 연결)
```bash
python automation/send_newsletter.py \
  --to 대표 메일 \
  --subject "이번 주, 모공 케어의 새 기준 🌿 A 브랜드 위클리" \
...
| Gmail MCP 가 발송 불가 (초안만 지원) | 메일 발송 도구(직접 연결) SMTP 경로로 전환 |
