...
> "부조종사가 아니라 팀이다." 2026-07-16 MSK(marketing 100 skills) 팀장 조직을 결합해 2계층으로.
> 방법론(스킬·게이트 룰)은 MSK 참조·복사 금지 / **브랜드 데이터(profile·tone·my-playbook)는 `marketing-os/brand/` 소유**(이 회사 배치 정본 · MSK 촬영 리셋과 무관) / 실행력(gbrain·MCP·도구)은 marketing-os.
...
트루먼 (오케스트레이터 · agents/orchestrator.md · SSOT)
   ...
   │      └ 각 팀장은 담당 Part 실행 에이전트를 도구로 사용
   │
   ├─ 스태프 3 (agents/staff/)
   │   perspective-reviewer (5관점 병렬·sonnet) · gate-auditor (유일 ⛔) · skill-builder (제작 전담)
   │
   ├─ Part0 운영팀 (agents/part0-ops/) · 사용자 일상 운영 (채널브리핑·기획·감사 등)
   └─ Part3~9 실행 에이전트 28 (팀장이 지휘하는 실행자, canonical_skill 배선)
```

## 팀장 → Part 실행 에이전트 매핑
| 팀장 | 담당 Part 실행 에이전트 | 직속 없는 스킬(정본 직접 실행) |
|---|---|---|
...
- **업무 브리핑 게이트**(§E): 실행 전 팀장 명의 1화면 계획→⏸ / **결과확인+개선 루프**(§F)
- **실데이터 우선**(§G): 측정 도구 있으면 추정 금지 / **권한 분리**(§H): 제작·품질·규제 상호 대행 불가
- **핸드오프 계약**(§B) · **gbrain 복리**(§A) · **게이트 기본 반려**(§D)

## 협업 5원칙
1. 피드포워드(앞 산출물=다음 입력) 2. 단일 보고선(팀장→트루먼만, 사용자 대면은 트루먼 조립)
3. 권한 분리(⛔ 번복은 사용자만) 4. 병렬 우선 5. 기록 의무(build-log 1행/산출물)

## 확장·모델
- 팀장·스태프 추가는 이 문서 + agents/leads|staff/ 동시 등록(한쪽만=유령 조직)
- perspective-reviewer = `model: sonnet`(판정 전용·5병렬, 비용 최적화). 나머지 = 세션 모델 상속

## 드리프트 룰 (MSK 원본 13과의 관계)
- leads/·staff/ 13 = MSK `.claude/agents/` 13의 **이 회사 배치판** (각 파일 frontmatter `origin:` 참조)
- 조직 **설계** 변경(역할·권한·브리핑 규칙) → **MSK에서 먼저** 수정(제품 정본) 후 여기 재적용
- 이 회사 **배선** 변경(Part 매핑·gbrain·도구) → 여기서만, MSK 역류 금지 (상품 순수성)
