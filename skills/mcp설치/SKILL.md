...
name: mcp설치
description: |
  MCP 1개를 설치·검증·활용하는 4단계 표준 파이프라인. 마케터(비개발자) 기준 인터랙티브 진행. 8개 도구 지원 (Google Sheets · GA4 · Firecrawl · YouTube · Buffer · Meta/Google Ads · Notion · Discord).
...
> 12개 도구 모두 동일한 4단계 흐름으로 설치·검증·활용. 마케터가 어떤 MCP를 처음 만나도 같은 패턴으로 진입 가능.
...
🎉 첫 결과물 완성. 이걸 발전시키는 3가지 경로:
  ...
  A. 정기 자동화: 이 작업을 매일/매주 자동 실행 (AX 에이전트)
  B. 다른 MCP 결합: {제안 MCP} 와 함께 쓰면 {새 작업} 가능
  C. 다른 MCP 설치: 다음 가이드 진행
```
...
- 본 스킬은 12개 클립 모두에서 호출되는 공통 설치 파이프라인입니다.
- 각 클립의 후반부 "설치 실습" 섹션에서 이 스킬을 호출.
- video-renderer 같은 자동화 에이전트가 본 스킬로 설치된 MCP 들을 활용.
