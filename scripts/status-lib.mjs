...
export function scanSessions() {
  ...
  for (const d of dirs) {
    ...
    const label = cwd ? cwd.split('/').filter(Boolean).slice(-2).join('/') : d.replace(/^-Users-사용자-/, '');
...
export function scanInventory() {
  ...
  // 방법론 100 스킬 (100-skills/NN-category/skills/NNN-slug/SKILL.md)
  // skills/ 는 대시보드·OS 운영용 스킬(47개)이고, 이쪽이 "마케팅 100 스킬" 정본이다.
  const methods = [];
  try {
    for (const cat of fs.readdirSync(path.join(ROOT, '100-skills'), { withFileTypes: true })) {
      if (!cat.isDirectory()) continue;
      const skillsDir = path.join(ROOT, '100-skills', cat.name, 'skills');
      let entries = [];
      ...
      for (const e of entries) {
        ...
        let fm = null;
        try { fm = parseFrontmatter(fs.readFileSync(sf, 'utf8').slice(0, 4000)); } catch { continue; }
        if (!fm) continue;
        methods.push({
          id: String(fm.id ?? '').padStart(3, '0') || e.name.slice(0, 3),
          name: typeof fm.name === 'string' ? fm.name : e.name,
          desc: typeof fm.description === 'string' ? fm.description.slice(0, 300) : '',
          slug: typeof fm.slug === 'string' ? fm.slug : e.name,
          category: typeof fm.category === 'string' ? fm.category : cat.name,
          tier: typeof fm.tier === 'string' ? fm.tier : '',
          file: path.relative(ROOT, sf),
        });
      }
    }
