...
description: |
  ...
  특이점: 둘 다 공식 MCP (2026-05 신규 출시). Meta 는 Meta 가 직접 운영하는 hosted MCP. Google 은 Google 이 직접 유지보수하는 GitHub MCP · 읽기 전용 강제 (캠페인 변경 불가, 실수 위험 0). ADC 는 GA4 가이드과 공유.
---

# Meta·Google Ads MCP 설치
> 본 스킬은 Meta + Google Ads MCP 2 종을 본인 광고 계정 기준 즉시 운영 가능한 흐름으로 설치하고 매체 통합 ROAS 비교 + 예산 재배분 시뮬레이션 1건을 시연하는 흐름. 마스터 스킬 `mcp설치` 의 4단계 표준을 2 MCP 에 순차 적용한 것.
...
- **3매체 통합 ROAS 리포트** · Meta + Google + Naver 한 표로 비교 (양 매체 조회)
...
> ADC 는 GA4 가이드과 같은 파일 공유 (`~/.config/gcloud/application_default_credentials.json`). 단, **`adwords` scope 가 포함돼 있어야** 합니다. GA4 때 analytics scope 만으로 로그인했다면 아래 한 줄로 재로그인 (공식 README 권장 명령):

```bash
gcloud auth application-default login \
  --scopes https://www.googleapis.com/auth/adwords,https://www.googleapis.com/auth/cloud-platform \
  --client-id-file=<본인 OAuth 데스크톱 클라이언트 JSON>   # GA4 가이드에서 만든 파일 재사용
```
...
본 STEP 의 핵심: **활성 캠페인이 없어도 연결 자체는 정상 작동** 검증 + 본인 캠페인 데이터 없이도 보여줄 수 있는 와우 모먼트 (경쟁사 광고 라이브러리) + 캠페인 시작하는 날 즉시 자동화 가능하도록 도구·쿼리 학습. 광고 에이전트로 이어지는 진입점.

> 📌 **실측 검증 노트 (2026-06-11 · 본 계정 기준)** — 시연 기대값:
> - 시연 A 헬스 체크 ✅ : **이 회사 한정 조회** — Meta "이 회사 마케팅" (활성·queryable·KRW) · Google "이 회사" MCC + 일반 계정 (KRW)
> - 시연 B 라이브러리 ✅ : "화장품" KR 검색 → 약 2,900건 · 아토팜·딘토·무신사 실광고 + 스냅샷 URL
> - 시연 C GAQL ✅ : 이 회사 Google 계정 GAQL 쿼리 실행 (캠페인 0건 응답도 정상 — 연결 검증)
> - ⚠️ 성과 지표 (spend·CTR·ROAS) 와 기회 점수는 **활성 캠페인이 없어 "Not available"** → 본 계정으로는 시연 불가. ROAS 표 시연은 캠페인 활성화 이후로 약속할 것.
>
> 🔒 **녹화 프라이버시 규칙 (필수)**: 연결된 Meta 계정에 **타사(클라이언트) 광고 계정 다수 포함** → 녹화에서 "내 광고 계정 전부 보여줘" 류의 **전체 조회 명령 금지**. 반드시 "**이 회사 광고 계정만 보여줘**" 로 한정하고, Claude 는 시연 중 이 회사 외 계정명·ID 를 화면에 출력하지 않는다.

### 4.1 시연 A · 2 MCP 연결 헬스 체크 (약 1분)

```
사용자: "이 회사 광고 계정만 보여줘. Meta·Google 둘 다.
        통화 + 캠페인 수 + 사용 가능 도구 확인. 다른 계정은 출력하지 마."
```

자동 실행 (🔒 이 회사 외 계정명·ID 는 결과에서 필터링):

```
1. Meta hosted · 광고 계정 조회 → business_name == "이 회사" 만 추려서 표시
   → "이 회사 마케팅" · 통화 KRW · 활성 캠페인 (0개여도 정상 응답)

2. mcp__google-ads__customers_list_accessible_customers
   → Customer 목록 중 이 회사 관련만 표시 (MCC + 일반 계정)

3. mcp__google-ads__search_search (검증 GAQL · 이 회사 일반 계정):
   SELECT customer.id, customer.descriptive_name
   FROM customer LIMIT 1
   → 권한·접근 확인 (Developer Token 등급도 여기서 검증됨)

4. 결과 표 출력:
   ✅ Meta Ads · 이 회사 마케팅 · 통화 KRW · 캠페인 0개
   ✅ Google Ads · 이 회사 (MCC) + 일반 계정 · 통화 KRW · 캠페인 0개
...
- `research-ad-references` 에이전트 (광고 소재 수집 → Sheets) 와 연결되는 진입점이라고 안내
...
```
사용자: "이 회사 Google Ads 계정에서 캠페인 목록을 GAQL 로 조회해줘"
```
...
> 🔒 강사 녹화 시에는 이 전체 조회 명령 대신 **"이 회사 광고 계정만 보여줘"** 사용 (타사 계정 노출 방지 · STEP 4 프라이버시 규칙 참조). 사용자은 본인 계정뿐이므로 전체 조회 그대로 OK.
...
→ 약 30초. 집행 중 실광고 수천 건 + 브랜드·카피·스냅샷 URL. **본인 캠페인 0개여도 동작** — 광고 안 돌리는 사용자의 첫 성공 경험으로 최적.
...
[입력]
- Meta + Google 광고 계정 (Naver 는 나중에 추가)
...
4. **광고 에이전트들 (`meta-ads-analyzer` 등) 이 본 MCP 를 자동 호출** · 캠페인 시작하는 날 즉시 자동화 시작
...
- 본 스킬로 설치된 MCP 는 **광고 에이전트들이 자동 호출**:
...
- 광고 자동화 에이전트가 매시 cron 으로 임계치 모니터링 + 매주 월요일 통합 리포트.
...
- 본 스킬 종료 후 사용자가 "매시 임계치 알림 자동" 이라고 하면 `ad-performance-checker` 에이전트 (`/check-ads`) 또는 cron 구성으로 전달
