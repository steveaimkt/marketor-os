...
```
[기본 3]                              [고도화 5 · 자산]
  A. 📅 일정·메일 매니저 ⭐ 추천 D. 🔬 리서치 인텔리전스
  B. 📊 마케팅 매니저 E. 📝 콘텐츠 파이프라인
  C. ✍️ 콘텐츠 매니저 F. 💰 광고 옵스
                                        G. 📊 GA4 데이터 분석
[자유 설계]                            H. 🎯 CRM 리텐션
...
6. **구현 우선순위** — 오늘 (daily-briefing 첫 실행) / 1주 (launchd+cron+권한) / 1개월 (orchestrator · )
...
- `discord-channels-setup` STEP 11-3 직후가 자연스러운 호출 시점. 도출된 3개 에이전트 = 학습자의 첫 "AX 팀" (게이트 역할)
