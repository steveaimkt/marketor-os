...
name: mcp설치-buffer
description: |
  Buffer MCP 설치 스킬. Personal Access Token 발급 + .mcp.json 등록 (`@damusix/buffer-mcp` 패키지) 을 5~10분 안에 완료하고 신제품 5채널 동시 예약 1건을 자동 생성. 마케터(비개발자) 기준 4단계 표준 흐름.
...
# Buffer MCP 설치
> 본 스킬은 Buffer MCP 를 Personal Access Token 방식으로 설치하고 5채널 동시 예약 1건을 자동 생성하는 흐름. 설치 4단계 표준을Buffer 1 도구에 적용한 것이다.
...
4. **콘텐츠의 `content-publisher` 에이전트가 본 MCP 를 자동 호출** · 미리 채널 3개 이상 연결해 두기
...
- 본 스킬로 설치된 MCP 는 `content-publisher` 에이전트가 이어받아 활용 (콘텐츠 승인 → 자동 예약).
- `weekly-content-publisher` 같은 자동화 에이전트가 본 스킬로 설치된 MCP 를 매주 월요일 cron 으로 호출.
...
- 본 스킬 종료 후 사용자가 "매주 자동 발행" 이라고 하면 `content-publisher` 에이전트 또는 `/agent-builder` 스킬로 전달
