...
> 한 번 호출로 9개 MCP 를 **진행 순서**대로 순차 자동 설치. 사용자는 각 사이트에서 키 발급 또는 Claude.ai 클릭만 하고 Claude 에게 던지면 끝.
> (12개 = 11개 설치 스텝 · Step 6 광고가 Meta+Google 2개. 영상제작 Step 7 은 도구 3개 묶음.)

## 🧭 0단계 · 운영 모드 선택 (호출 즉시 가장 먼저 질문)

> ⚠️ **중요**: 본 스킬은 사용자가 이미 12개를 다 설치해 둔 상태에서도 자주 호출됩니다 (녹화·복습).
...
🚀 marketing-os MCP 통합 설치 스킬입니다. 먼저 진행 모드를 골라주세요.
  ...
  B. 교육 시연 모드 ⭐ (전체 과정 설명) — "이미 다 깔려 있어도 전 과정을 보여줘"
     - 9개 MCP 를 순서대로 하나도 빠짐없이 1개씩 설명·시연.
...
> 사용자가 "전체 과정 보여줘 / 다 설명해줘 / 녹화 / 시연 / 복습" 등으로 호출하면 **B 교육 시연 모드**로 간주하고 위 질문 없이 바로 B 로 진입해도 됨.

---

## 🎓 교육 시연 모드 동작 규칙 (B 모드)

> **목적**: 이미 12개가 다 설치된 PC 에서도, **설치하는 모든 과정을 처음부터 끝까지** 화면에 보여준다 (녹화용).
...
먼저 짚고 갈 게 한 가지 있어요:
  ...
  본 스킬은 9개 MCP 를 진행 순서 (1-2 → 4-2) 대로 셋업합니다.
...
총 8단계(=9개 MCP · Step 6 가 Meta+Google 2개)로 진행돼요 (사용자 작업 약 54분, Google Ads 1~2일 대기 별도):
  ...
  Ch 3. 배포와 광고 2개
  ✋ Step 5 Buffer (5분) Access Token                    [3-1]
  ✋ Step 6 Meta + Google Ads (6분+대기) hosted + Developer Token   [3-2]
...
먼저 `mcp-setup/00-사전준비물.md` 의 체크리스트를 완료하셨나요?
...
- `y` → Step 1 부터 8까지 전체 순차 (사전 준비 묻지 않음)
...
## ✋ Step 1 · Google Sheets OAuth (10분 · ★★ · [])
...
자동 처리:
```bash
mv ~/Downloads/oauth_credentials.json "${CLAUDE_PROJECT_DIR}/mcp-servers/google-sheets/"
cd "${CLAUDE_PROJECT_DIR}/mcp-server" && npm install

# 검증
grep -q '"installed"' "${CLAUDE_PROJECT_DIR}/mcp-servers/google-sheets/oauth_credentials.json" \
...
## ✋ Step 2 · GA4 Service Account (15분 · ★★ · [])
...
# ⚠ Workspace 정책이 서비스 계정 키 발급을 막는 경우(예: 회사 도메인) → ADC 경로:
...
## ✋ Step 6 · Meta + Google Ads (6분 + 1~2일 대기 · ★★ → ★ · [])
...
예상 출력:
```
🩺 marketing-os MCP 헬스 체크 (9개 MCP)

Ch 1. 데이터 분석
  ✅ google-sheets Connected · 10 tools · token.json valid
  ✅ ga4 Connected · 5 tools · property ${GA4_PROPERTY_ID}
