...
name: mcp설치-notion
description: |
  (Notion MCP) 설치 스킬. Claude.ai OAuth 통합 활성화 + 노션 페이지 공유 (Add connections) 를 5~7분 안에 완료하고 콘텐츠 캘린더 30개 카드 자동 등록 1건까지 진행. 설치 마라톤(mcp설치-전체) Step 10 Notion 단계의 상세 버전이자 단독 (재)설치용 참조 스킬. 마케터(비개발자) 기준 4단계 표준 흐름.
...
# Notion MCP 설치
> 본 스킬은 Notion MCP 를 Claude.ai OAuth 방식으로 활성화하고 콘텐츠 캘린더 30개 카드를 자동 등록하는 흐름. 설치 4단계 표준을Notion 의 OAuth 통합 패턴에 적용한 것이다.
...
- **광고 리포트 아카이브** · 광고 에이전트가 매주 자동 게시
- **브랜드 가이드 자동 작성** · `brand-guideline` 에이전트가 보이스·톤·페르소나 페이지 생성
- **고객 응대 위키 검색** · `cs-responder` 가 위키 검색으로 정확한 답변
...
🎉 첫 캘린더 30개 등록 완성. 이걸 발전시키는 4가지 경로:
  ...
  A. 다른 시나리오 시도:
     ...
     - "브랜드 가이드 페이지 작성" (Case C · brand-guideline)
...
→ 약 1~2분. 광고 에이전트들이 이 DB 에 매주 자동 게시.
...
→ 약 2분. `brand-guideline` 에이전트가 이 패턴으로 자동 생성.
...
4. **콘텐츠·카피에서 `brand-guideline` 에이전트가 본 MCP 를 자동 호출** · 미리 "브랜드 가이드라인" 페이지 위치 정해두기
...
-  본편(4-1-notion-5min.md · 5분 이론)에서는 본 스킬을 호출하지 않습니다. 슬라이드 06 은 "이론 끝 · 시연 시작" 안내이며, 설치는 마지막 **설치 마라톤** 영상(skills/mcp설치-전체/SKILL.md Step 10)에서 일괄 진행됩니다.
- 본 스킬의 역할 2가지: ① 설치 마라톤 Step 10 (Notion) 의 상세 절차 ② Notion 만 단독 (재)설치할 때의 참조 스킬.
- 마스터 스킬 skills/mcp설치/SKILL.md 의 4단계 표준 흐름을 Notion 의 OAuth 통합 패턴에 적용한 버전.
- 본 스킬로 설치된 MCP 는 `content-publisher` · `brand-guideline` · 광고 리포트 에이전트 · `cs-responder` 가 자동 호출.
- `weekly-calendar-builder` 같은 자동화 에이전트가 본 스킬로 설치된 MCP 를 매주 cron 으로 호출.
- 참조 자산: 교육 파트너 프로젝트 (2)
...
- 본 스킬 종료 후 사용자가 "매주 캘린더 자동 갱신" 이라고 하면 `content-publisher` 에이전트 또는 `/agent-builder` 로 전달
