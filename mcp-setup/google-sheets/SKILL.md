...
description: |
  Google Sheets MCP 설치 스킬. OAuth 2.0 자격증명 발급부터 자체 호스팅 MCP 서버 등록·브라우저 인증·연결 확인까지 15~20분 안에 완료. 마케터(비개발자) 기준 4단계 표준 흐름.
...
# Google Sheets MCP 설치
> 본 스킬은 Google Sheets MCP 1 도구를 OAuth 2.0 방식으로 설치하고 첫 자동 리포트 1건을 만드는 흐름. 마스터 스킬 `mcp설치` 의 4단계 표준을 적용한 것.
...
test -f "${CLAUDE_PROJECT_DIR}/mcp-servers/google-sheets/index.js" && echo "OK" || echo "MISSING"

# 5) 기존 설치 상태 확인 (재진입 시 빠른 처리)
test -f "${CLAUDE_PROJECT_DIR}/mcp-servers/google-sheets/oauth_credentials.json" && echo "creds:OK" || echo "creds:NEED"
test -f "${CLAUDE_PROJECT_DIR}/mcp-servers/google-sheets/token.json" && echo "token:OK" || echo "token:NEED"
test -d "${CLAUDE_PROJECT_DIR}/mcp-servers/google-sheets/node_modules" && echo "deps:OK" || echo "deps:NEED"
```
...
| 제공사 | 자체 호스팅 (mcp-servers/google-sheets/index.js · @modelcontextprotocol/sdk + googleapis) |
...
> ⚡ marketing-os 에는 **검증된 서버 코드(mcp-servers/google-sheets/index.js · 10 도구 + OAuth2 구현)** 가 이미 포함되어 있어요. 작성 단계 없이 의존성만 설치합니다.

#### 2.2.1 사전 검증

```bash
# 서버 코드 존재 확인
test -f "${CLAUDE_PROJECT_DIR}/mcp-servers/google-sheets/index.js" \
  && echo "✓ 서버 코드 존재" \
  || echo "✗ marketing-os 손상 — git pull 필요"

# package.json 무결성
cat "${CLAUDE_PROJECT_DIR}/mcp-servers/google-sheets/package.json" | grep -E "modelcontextprotocol|googleapis"
```
...
설치되는 패키지 (`mcp-servers/google-sheets/package.json`):
...
서버 코드 구현 (`mcp-servers/google-sheets/index.js` · 약 480줄):
...
`.mcp.json` 의 `mcpServers` 안 google-sheets 항목 확인:

```json
"google-sheets": {
  "_part": "2 Ch1-2 스프레드시트 자동화 (자체 호스팅 MCP)",
  "command": "node",
  "args": ["${CLAUDE_PROJECT_DIR}/mcp-servers/google-sheets/index.js"]
}
```
...
# 출력 예: google-sheets: ... node mcp-servers/google-sheets/index.js - ✓ Connected
```
...
성공 시 `mcp-servers/google-sheets/token.json` 이 자동 생성됩니다.
...
```bash
claude mcp list 2>&1 | grep google-sheets
# 기대 출력: google-sheets: node ... mcp-servers/google-sheets/index.js - ✓ Connected
```

`Connected` 가 아니면:
- `Failed to connect` → mcp-servers/google-sheets/index.js 실행 에러. 다음 명령으로 로그 확인:
  ```bash
  node "${CLAUDE_PROJECT_DIR}/mcp-servers/google-sheets/index.js" 2>&1 | head -20
  ```
...
설치 직후 확인:
- [ ] `mcp-servers/google-sheets/oauth_credentials.json` 이 `.gitignore` 에 등록됨
- [ ] `mcp-servers/google-sheets/token.json` 이 `.gitignore` 에 등록됨
...
본 스킬이 끝나면 다음 7개가 모두 만족되어야 완료:

| # | 검증 항목 | 확인 방법 | 통과 기준 |
|---|---|---|---|
| 1 | OAuth 자격증명 발급 | `cat mcp-servers/google-sheets/oauth_credentials.json \| grep installed` | `"installed"` 키 존재 |
| 2 | 의존성 설치 | `ls mcp-servers/google-sheets/node_modules/@modelcontextprotocol` | 폴더 존재 |
| 3 | .mcp.json 항목 | `grep google-sheets .mcp.json` | `node mcp-servers/google-sheets/index.js` 라인 표시 |
| 4 | MCP 서버 연결 | `claude mcp list \| grep google-sheets` | `✓ Connected` 표시 |
| 5 | OAuth 토큰 발급 | `ls mcp-servers/google-sheets/token.json` | 파일 존재 (60일 유효) |
...
🎉  Google Sheets MCP 설치 완료 (총 14~16분)
```
...
>  실습.md 와 본 스킬을 함께 운영. 본 섹션은 진행 시 시연용 명령·5패턴·응용 과제.
...
2. **에서 Meta 광고 분석 에이전트가 자동으로 이 시트에 데이터를 적재** · 미리 시트 만들어 두기
...
| `Error: ENOENT: oauth_credentials.json` | 파일 위치 오류 | 파일이 `mcp-servers/google-sheets/` 폴더 안에 있는지 확인 |
...
| `Token has been expired` (60일 미사용) | OAuth 토큰 만료 | `rm mcp-servers/google-sheets/token.json` 후 Claude 재시작 → 재인증 |
...
- `weekly-sheets-reporter` 같은 자동화 에이전트가 본 스킬로 설치된 MCP 를 cron 스케줄로 호출.
...
| 자체 호스팅 서버 위치 | `marketing-os/mcp-servers/google-sheets/index.js` |
| OAuth 자격증명 파일 | `mcp-servers/google-sheets/oauth_credentials.json` (gitignore) |
| OAuth 토큰 파일 | `mcp-servers/google-sheets/token.json` (gitignore, 자동 생성) |
...
- 본 스킬 종료 후 사용자가 "주간 리포트 자동화 등록하자" 라고 하면 `/agent-builder` 스킬로 전달
