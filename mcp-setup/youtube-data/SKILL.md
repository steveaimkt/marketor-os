...
name: mcp설치-youtube
description: |
  YouTube Data MCP 설치 스킬. Google Cloud 프로젝트 + YouTube Data API v3 활성화 + API Key 발급 + (선택) 키 제한 + 본인 채널 ID + .mcp.json 등록 (`youtube-data-mcp-server` 패키지) 을 8~10분 안에 완료하고 키워드 검색 헬스 체크 + 가능한 분석 8가지 정리. 마케터(비개발자) 기준 4단계 표준 흐름.
  ...
  특이점: API Key 단일 인증 방식 (GA4 의 Service Account JSON 보다 간단). 공개 데이터(채널·영상·댓글 읽기)는 API Key 만으로 가능. 본인 채널 댓글 쓰기는 OAuth 필요 (별도). trend-scanner + 콘텐츠 기획 에이전트 기반 MCP.
---

# YouTube Data MCP 설치
> 본 스킬은 YouTube Data MCP 를 API Key 단일 인증 방식으로 설치하고 키워드 검색 헬스 체크 + 가능한 분석 8가지를 안내하는 흐름. 설치 4단계 표준을YouTube 의 API Key + Data API v3 패턴에 적용한 것이다.
...
사전 점검 5가지부터:
  ...
  □ Google Cloud 프로젝트 (GA4 가이드에서 만든 거 재사용 가능)
...
→ Analytics API 의 OAuth 셋업은 `../analytics-api-oauth.md` Phase 2 (약 15분) 에서 진행. 본 스킬은 Phase 1 (Data API v3 + API Key) 만 끝까지 다룸.
...
| # | 제목                          | 채널         | 조회수    | 업로드일   |
|---|------------------------------|--------------|-----------|-----------|
| 1 | Claude Code 마케팅 자동화...  | 대표 채널  | 28,400   | 2026-05-12|
...
- [ ] 본인 채널 댓글 쓰기·답글이 필요한 경우 OAuth 2.0 별도 설치 안내 (이번 가이드은 읽기만)
...
본 MCP 는 **trend-scanner + 콘텐츠 기획 에이전트의 기반** · 콘텐츠 분석 파트의 핵심.

---

## 🎯 STEP 4: 결과물 1개 (연결 확인) + 가능한 분석 8가지

본 STEP 의 핵심은 **설치가 정상적으로 작동하는지 헬스 체크 한 줄로 확인**한 뒤, 이제 어떤 분석이 가능한지 카탈로그를 보여주는 것. 본격 활용은 에이전트에서.
...
이 한 줄이 응답하면 **설치 검증 완료**. 본격 분석은 에이전트에서.
...
🎉 YouTube Data MCP 설치 + 연결 확인 완료. 다음 가능합니다:
  ...
  C. 에이전트 · 위 8가지를 자동화:
     - trend-scanner: 키워드 트렌드 매일 자동 추적
     - youtube-content-analysis: 주간 영상 성과 자동 리포트
     - youtube-comment-responder: 댓글 자동 분류 + 답글 우선순위
...
>  실습.md 와 본 스킬을 함께 운영. 본 섹션은 진행 시 시연용 명령·5패턴.
...
| 댓글 쓰기·답글 안 됨 | API Key 만으로는 불가 (읽기 전용) | OAuth 2.0 인증 별도 설정 (이 가이드 범위 외) |
...
- 본 스킬로 설치된 MCP 는 **콘텐츠 에이전트의 기반**:
- 다른 활용 에이전트:
  - · `orchestrator` · "/weekly-content" 슬래시 명령 라우팅
- 참조 자산: 교육 파트너 프로젝트 (2)
  - `.mcp.json` (mcp-server-youtube 등록 예시)
...
- 본 스킬 종료 후 사용자가 "댓글 자동 답글" 이라고 하면 OAuth 별도 설정 가이드 안내 (이 가이드 범위 외)
- 사용자가 "트렌드 매일 추적" 이라고 하면 `trend-scanner` 에이전트로 전달
- 사용자가 "주간 콘텐츠 리포트" 라고 하면 `youtube-content-analysis` 에이전트로 전달
