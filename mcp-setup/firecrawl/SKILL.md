...
description: |
  Firecrawl MCP 설치 스킬. API key 발급 + .env 등록 + .mcp.json (`firecrawl-mcp` 패키지) + 헬스 체크를 약 5분 안에 완료하고 경쟁사 3개 도메인 신제품 비교표 자동 생성 1건을 시연. 마케터(비개발자) 기준 4단계 표준 흐름.
  ...
  특이점: Cloud API 방식이라 노트북 부담 없음. 무료 500크레딧/월 학습용 충분. AI 친화 마크다운 출력. 본 에이전트 중 약 1/3 (리서치·콘텐츠·전략 계열) 이 호출.
---

# Firecrawl MCP 설치
> 본 스킬은 Firecrawl MCP 를 Cloud API key 방식으로 설치하고 경쟁사 3개 도메인 신제품 비교표 자동 생성을 시연하는 흐름. 설치 4단계 표준을Firecrawl 의 API key 패턴에 적용한 것이다.
...
> ⚠️ **윈도우 사용자 분기 (필수)**
>
> 위 설정은 macOS 전용. 윈도우에서 `"command": "npx"` 를 그대로 쓰면 MCP 서버가 조용히 죽어 도구 목록에 안 잡힌다 (윈도우의 npx 는 실제로 `npx.cmd` 라서 셸 없이 스폰하면 못 찾음 · 공식 문서도 `cmd /c` 래핑 요구). 또한 윈도우는 `.env` 자동 로드가 없어 `${FIRECRAWL_API_KEY}` 가 빈 값이 된다. 윈도우는 아래로 등록:
>
> ```json
> "firecrawl": {
>   "_part": "2 Ch 1-4 경쟁사·시장 정보 자동 수집",
...
>   "env": {
>     "FIRECRAWL_API_KEY": "fc-실제키를여기에"
>   }
> }
> ```
>
> 또는 PowerShell 한 줄 등록 (프로젝트 폴더에서 실행해야 함):
>
> ```powershell
> claude mcp add firecrawl --env FIRECRAWL_API_KEY=fc-실제키 -- cmd /c npx -y firecrawl-mcp
> ```
>
> 수정 후 IDE 완전 종료 → 재실행 → 프로젝트 MCP 승인 프롬프트에서 "예". 증상 진단: `claude mcp list` 에서 `✗ Failed` 면 대부분 `cmd /c` 누락 또는 키 오타.
...
본 MCP 는 **에이전트 중 약 1/3 (리서치·콘텐츠·전략 계열) 이 호출** · 본 핵심 자산.
...
- 경쟁사 블로그를 매주 모니터링하는 워크플로 설계 (에서 자동화)
...
- 본 스킬로 설치된 MCP 는 **에이전트 중 약 1/3 (리서치·콘텐츠·전략 계열) 이 자동 호출** · 본 핵심 자산.
- 주요 활용 에이전트:
...
본 채택은 Firecrawl 이나, 사용자가 특정 상황에 부닥치면 다음 대안 검토:
...
- 본 스킬 종료 후 사용자가 "매일 자동으로 돌리고 싶다" 라고 하면 `competitor-monitor` 에이전트로 전달
